...
* Start the base node for the Bunker robot
    ...
    $ ros2 launch bunker_base bunker_base.launch.py \
        odom_topic_name:=/wheel/odom publish_odom_tf:=false
    ```

  In the AGT field integration, the launch file remaps the driver's internal
  `/cmd_vel` subscription to `/mux/cmd_vel`. Do not publish directly to the
  driver's original command topic; use the fail-closed command guard path.
